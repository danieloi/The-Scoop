...
function addComment(url, request) {
  ...
  //the assignment statement above asigns the first non-truthy value on the RHS to the LHS
  //if all values are truthy, it evalues to the rightmost value.
...
function editCommment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
